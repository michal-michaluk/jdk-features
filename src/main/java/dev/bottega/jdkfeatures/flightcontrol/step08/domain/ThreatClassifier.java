...
public final class ThreatClassifier {
    ...
    /**
     * Creates a classifier with the given alarm speed threshold and the sector (control zone).
     *
     * @param alarmMinSpeed threshold; an aircraft is `ALARM` when its scalar speed is strictly above it
     * @param sector        the containment circle used by {@link #classify(Aircraft)}
     */
    ...
    /** The alarm speed threshold (strictly greater than ⇒ `ALARM`, strictly less/equal ⇒ not). */
    public double alarmMinSpeed() {
        return alarmMinSpeed;
    }

    /** The sector (control zone) used by {@link #classify(Aircraft)} as the containment circle. */
